...
namespace DTIMS.DataAcess.Common
{
    /// <summary>
    /// SysLog 的摘要说明。
    /// </summary>
    public class SysLog
    {
        ...
        {
            Database mDataBase = DatabaseFactory.CreateDatabase(SystemWebFormulation.Parameter("MainDataBaseName"));
        ...
        #endregion
        #region 黑百名单
        public static DataTable GetDataHbmdTable(string sfz,int pageSize, int page, string orderby, string startTime, string endTime, int hbMd, out int row_data, out int cur_page_index)
        {
            string where = "";
            if (!string.IsNullOrEmpty(startTime) && !string.IsNullOrEmpty(endTime))
            {
                where = string.Format("TESTDATE between '{0} 00:00:00' and '{1} 23:59:59'", startTime, endTime);
            }

            //判断是否需要按帐号查询 PASS 1 是黑名单 0是白名单
            if (hbMd > -1)
            {
                //where = " PASS='" + hbMd + "' AND " + where;
            }

            if(sfz != "")
                where = " SFZMHM like '%" + sfz + "%' AND " + where;

            DataTable dt = null;
            try
            {
                string field = " [ID],[SFZMHM],[NAME],convert(char(10),TESTDATE,120) TESTDATE,[LSH],[KSKM],[JFBJ],[KSCS],[KZCS],[PASS] ";
                ...
                DTIMS.DataAccess.Common com = new DTIMS.DataAccess.Common();
                dt = com.Query(pageSize, page, tableName, "ID", field, where, orderby, out row_data, out cur_page_index);
            }
            catch (Exception e)
            {
                throw (new Exception(e.Message));
        ...
        public static DataTable GetSubjectLog(int pageSize, int page, string orderby, string startTime, string endTime, int hbMd, out int row_data, out int cur_page_index)
        {
            string where = "";
            if (!string.IsNullOrEmpty(startTime) && !string.IsNullOrEmpty(endTime))
            {
                where = string.Format("PASSTIME between '{0} 00:00:00' and '{1} 23:59:59'", startTime, endTime);
            }
            ...
            try
            {
                //string field = " [ID],[SFZMHM],[PASSTIME],[REASON],[REASONID] ,[PASS],[KSKM] ";
                string field = " [ID],[SFZMHM],[PASSTIME],[REASON],[REASONID] ,[PASS],[KSKM] ";
                string tableName = " T_PassLog ";
                if (string.IsNullOrEmpty(orderby))
                {
                    orderby = " ID DESC ";
                }
        ...
        public static DataTable GetDataHbmdLogTable(int pageSize, int page, string orderby, string startTime, string endTime, int hbMd, out int row_data, out int cur_page_index,string subject)
        {
            string where = "";
            string studentWhere = string.Empty;
            if (!string.IsNullOrEmpty(startTime) && !string.IsNullOrEmpty(endTime))
            {
                where = string.Format("PASSTIME between '{0} 00:00:00' and '{1} 23:59:59'", startTime, endTime);
            }
            ...
            if(!subject.Equals("-1"))
                studentWhere = " WHERE  KSKM='" + subject + "'  ";
            ...
            try
            {
                string field = string.Format(@" 
                                    ...
                                    ,P.PASSTIME
                                    ,P.REASON
                                    , CASE WHEN P.REASONID = 1 THEN '无考生信息'
                                    WHEN P.REASONID = 1 THEN '未交费'
                                    WHEN P.REASONID = 1 THEN '黑名单'
                                    WHEN P.REASONID = 1 THEN '人脸验证失败'
                                    END REASONID
                                    , CASE WHEN P.PASS = 0 THEN '通过' else '不通过' end PASS
                    ");
                string tableName = " T_PassLog  P inner JOIN (select * from t_student " + studentWhere + ")  S ON S.SFZMHM = P.SFZMHM AND convert(char(10),S.TESTDATE,120) = convert(char(10),P.PASSTIME,120) ";
                if (string.IsNullOrEmpty(orderby))
                {
                    orderby = " P.ID DESC ";
                }
                DTIMS.DataAccess.Common com = new DTIMS.DataAccess.Common();
                dt = com.Query(pageSize, page, tableName, "P.ID", field, where, orderby, out row_data, out cur_page_index);
            }
            catch (Exception e)
            {
                throw (new Exception(e.Message));
