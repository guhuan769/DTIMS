<%@ Page Language="c#" Inherits="Inphase.Project.CTQS.Descent.ChangePassword" CodeBehind="ChangePassword.aspx.cs" %>
    ...
    <script language="javascript" type="text/javascript">
        function docheck() {
            if (document.Form1.textBoxOldPwd.value.trim() == '') {
                $.messager.alert('错误', '请输入你的旧密码！', 'error', function () { document.Form1.textBoxOldPwd.focus(); });
                return false;
            }
            if (document.Form1.textBoxNewPwd.value.trim() == '') {
                $.messager.alert('错误', '请输入你的新密码！', 'error', function () { document.Form1.textBoxNewPwd.focus(); });
                return false;
            }
            ...
            if (document.Form1.textBoxNewPwd.value.trim() != document.Form1.textBoxConPwd.value.trim()) {
                $.messager.alert('错误', '新密码和确认密码不一致！', 'error', function () { document.Form1.textBoxNewPwd.focus(); });
                return false;
            }

            if (charValidate(document.getElementById('textBoxOldPwd').value)) {
                $.messager.alert('错误', '旧密码包含特殊字符！', 'error');
                return false;
            }

            if (charValidate(document.getElementById('textBoxNewPwd').value)) {
                $.messager.alert('错误', '新密码包含特殊字符！', 'error');
                return false;
            }

            if (charValidate(document.getElementById('textBoxConPwd').value)) {
                $.messager.alert('错误', '确认密码包含特殊字符！', 'error');
                return false;
            }
            ...
            <table width="780" border="0" cellspacing="0" cellpadding="0" class="table2" style="margin-top: 20px;">
                ...
                <tr>
                    <td height="30" align="right">
                        输入<span class="STYLE1">旧</span>密码：
                    </td>
                    <td height="30">
                ...
                <tr>
                    <td height="30" align="right">
                        确认<span class="STYLE1">新</span>密码：
                    </td>
                    <td height="30">
